...
- 建议 SVG 尺寸约 `1020x136`，对应顶部三张 `340x200` 卡片的合计宽度，并只保留 7 行贡献格所需高度
- README 中居中展示，宽度使用 `99%`，对齐顶部三张 `33%` 卡片
- 保留年度贡献格子的可读性，并在格子内显示每日具体贡献数
- 不展示标题、贡献总数、用户名、时间范围、更新时间、月份、星期或图例，只直接铺展贡献格
...
- 可见内容只保留贡献格和格子内数字，避免和顶部卡片标题、统计文本抢占视觉层级
...
- 根据每日贡献数映射热力等级
- 生成 `1020x136` 的宽幅纯网格 SVG
- 使用现有 Tokyo Night 色板
- 在每个贡献格中显示每日具体贡献数
- 不渲染标题、汇总统计、用户名、月份、星期、图例或生成时间
- 输出稳定、可 diff 的 SVG
