...
test("README and daily workflow publish the ordered two-by-two card layout", () => {
  ...
  // 用户策展的门面顺序：第一行 github-stats + blog，第二行 vlog + huggingface
  const cards = [
  ...
  assert.match(
    readme,
    /<p align="center">\n\s*<a href="https:\/\/www\.wangjinghong\.com\/" target="_blank"><img width="36%" src="\.\/assets\/github-stats-card\.svg" \/><\/a>&emsp;<a href="https:\/\/www\.wangjinghong\.com\/blog" target="_blank"><img width="36%" src="\.\/assets\/blog-card\.svg" \/><\/a>\n\s*<\/p>/,
  );
  assert.match(
    readme,
    /<p align="center">\n\s*<a href="https:\/\/www\.wangjinghong\.com\/vlog" target="_blank"><img width="36%" src="\.\/assets\/vlog-card\.svg" \/><\/a>&emsp;<a href="https:\/\/huggingface\.co\/ceilf6" target="_blank"><img width="36%" src="\.\/assets\/huggingface-card\.svg" \/><\/a>\n\s*<\/p>/,
  );
  assert.match(
    workflow,
    /- name: Fetch Hugging Face stats\n\s+id: fetch-huggingface\n\s+continue-on-error: true\n\s+run: python stats\/scripts\/fetch-huggingface-stats\.py/,
  );
